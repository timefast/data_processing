...
pd.concat([di_sudu,di_gaodu],ignore_index=True,axis=1).to_csv(datetime.now().strftime('%H-%M-%S') +'di.txt')

[zhong_sudu,zhong_gaodu] = [sudu_df.loc[zhong_index,:].max(),sudu_df.loc[zhong_index,:].idxmax()]
pd.concat([zhong_sudu,zhong_gaodu],ignore_index=True,axis=1).to_csv(datetime.now().strftime('%H-%M-%S') +'zhong.txt')

[gao_sudu,gao_gaodu] = [sudu_df.loc[gao_index,:].max(),sudu_df.loc[gao_index,:].idxmax()]
pd.concat([gao_sudu,gao_gaodu],ignore_index=True,axis=1).to_csv(datetime.now().strftime('%H-%M-%S') +'gao.txt')
...
jiliuzhishu_df.to_csv(datetime.now().strftime('%H-%M-%S') +'jiliuzhishu.txt')
...
pd.concat([sudu_12,gaodu_12],ignore_index=True,axis=1).to_csv(datetime.now().strftime('%H-%M-%S') +'12_height.txt')
...
qiangduzhishu_df.to_csv(datetime.now().strftime('%H-%M-%S') +'qiangduzhishu.txt')
...
pd.concat([half_di_sudu,half_di_gaodu],ignore_index=True,axis=1).to_csv(datetime.now().strftime('%H-%M-%S') +'half_di.txt')
pd.concat([half_gao_sudu,half_gao_gaodu],ignore_index=True,axis=1).to_csv(datetime.now().strftime('%H-%M-%S') +'half_gao.txt')
...
di_jiliuzhishu_df.to_csv(datetime.now().strftime('%H-%M-%S') +'di_jiliuzhishu.txt')
gao_jiliuzhishu_df.to_csv(datetime.now().strftime('%H-%M-%S') +'gao_jiliuzhishu.txt')
